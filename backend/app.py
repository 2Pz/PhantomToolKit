...
from backend.utils.config import get_base_dir, read_language, set_language
...
def list_builds():
    builds_dir = os.path.join(get_base_dir(), "builds")
...
def custom_static(filename):
    ...
    base_dir = get_base_dir()
    real_path = os.path.normpath(os.path.join(base_dir, "static", filename))
    data = None
    ...
    return send_file(io.BytesIO(data), mimetype=mime_type, max_age=86400)
